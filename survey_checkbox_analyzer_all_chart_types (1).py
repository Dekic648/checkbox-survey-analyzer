...
if uploaded_file:
    ...
    if not question_map:
        ...
        if segment_cols and selected_checkbox_cols:
            ...
            chart_type = st.radio("📊 Chart Type", options=["Stacked Horizontal", "Grouped Horizontal", "Stacked Vertical", "Grouped Vertical"], index=0)

            if chart_type == "Stacked Horizontal":
                chart = alt.Chart(chart_df).mark_bar().encode(
                    y=alt.Y('Segment:N', title='Segment', sort='-x'),
                    x=alt.X('Percent:Q', title='Checked (%)', stack='zero'),
            ...
            elif chart_type == "Grouped Horizontal":
                chart = alt.Chart(chart_df).mark_bar().encode(
                    y=alt.Y('Segment:N', title='Segment', sort='-x'),
                    x=alt.X('Percent:Q', title='Checked (%)'),
                    color=alt.Color('Option:N', legend=alt.Legend(title='Option')),
                    column=alt.Column('Option:N', title='Option'),
                    tooltip=[
                        alt.Tooltip('Segment:N', title='Segment'),
                        alt.Tooltip('Option:N', title='Option'),
                        alt.Tooltip('Count:Q', title='Checked Count'),
            ...
            elif chart_type == "Stacked Vertical":
                chart = alt.Chart(chart_df).mark_bar().encode(
                    x=alt.X('Segment:N', title='Segment', sort='-x'),
                    y=alt.Y('Percent:Q', title='Checked (%)', stack='zero'),
                    color=alt.Color('Option:N', legend=alt.Legend(title='Option')),
                    tooltip=[
                        alt.Tooltip('Segment:N', title='Segment'),
                        alt.Tooltip('Option:N', title='Option'),
